function Photopache(){return this.photos=[],this.directories=[],this.totalColumns=0,this.init(),this}Photopache.prototype={init:function(){var t=this;$("td.indexcolname > a").each(function(a){0!=a&&($(this).text().match(".[a-zA-Z]+$")?$(this).text().match("-thumbnail.[a-zA-Z]+$")||t.photos.push($(this).text()):t.directories.push($(this).text()))}),$("#indexlist").remove()},clearName:function(t){return t.replace(/[-_]/g," ").replace(/\.[a-zA-Z0-9]+/,"")},loadImageInModal:function(t){$(".modal-title > h1").text(this.clearName(this.photos[parseInt(t)])),$(".modal").attr("data-id",$(this).attr("data-id")).css("background-image","url("+encodeURI(this.photos[parseInt(t)])+")");var a=parseInt(t);0==t?($(".modal-next").attr("data-load",a+1),$(".modal-previous").attr("data-load",this.photos.length-1)):t==this.photos.length-1?($(".modal-next").attr("data-load",0),$(".modal-previous").attr("data-load",a-1)):($(".modal-next").attr("data-load",a+1),$(".modal-previous").attr("data-load",a-1))},findThumbail:function(t){return encodeURI(t.replace(/(\.[a-zA-Z]+)$/,function(t){return"-thumbnail"+t}))},generatePage:function(){$("<div></div>").addClass("pure-g").appendTo("body"),this.totalColumns=24;var t=$(window).width();1920>=t&&(this.totalColumns=12,1600>=t&&(this.totalColumns=8,1336>=t&&(this.totalColumns=6,1024>=t&&(this.totalColumns=4,640>=t&&(this.totalColumns=3,480>=t&&(this.totalColumns=2,320>=t&&(this.totalColumns=1)))))));for(var a=0;a<this.totalColumns;a++)$("<div></div>").addClass("pure-u-1-"+this.totalColumns).appendTo($(".pure-g"))},generateDirectories:function(){for(var t=0;t<this.directories.length;t++)$(".pure-menu-list").append('<li class="pure-menu-item"><a href="'+this.directories[t]+'" class="pure-menu-link">'+this.directories[t]+"</a></li>")},generatePictures:function(){for(var t=0;t<this.photos.length;t++)$(".pure-g > div").eq(t%this.totalColumns).append($("<div></div>").addClass("thumbnail").css("background-image","url("+this.findThumbail(this.photos[t])+")").attr("data-id",t));var a=this;$(".thumbnail").click(function(){$(".modal-cover").addClass("modal-visible"),a.loadImageInModal($(this).attr("data-id"))})},generateModal:function(){var t=this,a=function(){$(".modal-cover").removeClass("modal-visible")};$("body").append($("<div></div>").addClass("modal-cover").append($("<div></div>").addClass("modal").append($("<div></div>").addClass("modal-title").append($("<span></span>").addClass("modal-close").text("X").click(function(t){t.stopPropagation(),a()})).append($("<h1></h1>"))).append($("<div></div>").addClass("modal-arrow modal-previous").append($("<h1></h1>").text("←")).click(function(a){a.stopPropagation(),t.loadImageInModal($(this).attr("data-load"))})).append($("<div></div>").addClass("modal-arrow modal-next").append($("<h1></h1>").text("→")).click(function(a){a.stopPropagation(),t.loadImageInModal($(this).attr("data-load"))})).click(function(t){t.stopPropagation()})).click(function(t){t.stopPropagation(),a()}))},generateAll:function(){this.generatePage(),this.generateDirectories(),this.generatePictures(),this.generateModal()}};
