function Photopache(){return this.photos=[],this.directories=[],this.totalColumns=0,this.init(),this}Photopache.prototype={init:function(){var t=this;$("td.indexcolname > a").each(function(e){0!=e&&($(this).text().match(".[a-zA-Z]+$")?$(this).text().match("-thumbnail.[a-zA-Z]+$")||t.photos.push($(this).text()):t.directories.push($(this).text()))}),$("#indexlist").remove()},generatePage:function(){$("<div></div>").addClass("pure-g").appendTo("body"),this.totalColumns=24;var t=$(window).width();1920>=t&&(this.totalColumns=12,1600>=t&&(this.totalColumns=8,1336>=t&&(this.totalColumns=6,1024>=t&&(this.totalColumns=4,640>=t&&(this.totalColumns=3,480>=t&&(this.totalColumns=2,320>=t&&(this.totalColumns=1)))))));for(var e=0;e<this.totalColumns;e++)$("<div></div>").addClass("pure-u-1-"+this.totalColumns).appendTo($(".pure-g"))},generateDirectories:function(){for(var t=0;t<this.directories.length;t++)$(".pure-menu-list").append('<li class="pure-menu-item"><a href="'+this.directories[t]+'" class="pure-menu-link">'+this.directories[t]+"</a></li>")},generatePictures:function(){for(var t=0;t<this.photos.length;t++)$(".pure-g > div").eq(t%this.totalColumns).append($("<div></div>").addClass("thumbnail").css("background-image","url("+this.findThumbail(this.photos[t])+")").attr("data-name",this.clearName(this.photos[t])).attr("data-url",this.photos[t]));$(".thumbnail").click(function(){$(".modal-cover").addClass("modal-visible")})},generateModal:function(){$("body").append($("<div></div>").addClass("modal-cover").append($("<div></div>").addClass("modal").append($("<div></div>").addClass("modal-title").append($("<span></span>").addClass("modal-close").text("X").click(function(){$(".modal-cover").removeClass("modal-visible")})).append($("<h1></h1>"))).append($("<img />"))))},generateAll:function(){this.generatePage(),this.generateDirectories(),this.generatePictures(),this.generateModal()},clearName:function(t){return t.replace(/-([a-z])/,function(t){return" "+t}).replace(/-([a-z])/,function(t){return" "+t.toUpperCase()}).replace(/\/([^\/]*\/)+(.+)(\.\w+)+/,"$2")},findThumbail:function(t){return t.replace(/(\.[a-zA-Z]+)$/,function(t){return console.log("-thumbnail"+t),"-thumbnail"+t})}};
